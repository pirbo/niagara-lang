...
let make_acc (pinfos : ProgramInfo.t) (aggr : aggregate_eqs)
    (events : expr Variable.Map.t) =
  { pinfos; aggr; events; memo = Variable.Map.empty }

let find_vinfo t (v : Variable.t) =
  match Variable.Map.find_opt v t.pinfos.var_info with
  | None ->
      Report.raise_internal_error "Variable %d info not found" (Variable.uid v)
  | Some i -> i
...
let add_deps acc from events to_ =
  List.fold_left
    (fun acc from ->
      List.fold_left (fun acc to_ -> add_dep acc from events to_) acc to_)
    acc from

let add_eq acc dest eq = { acc with memo = Variable.Map.add dest eq acc.memo }
...
type expr_form = Direct of expr | Merge of Variable.t list
...
let result_to_var { acc; form; cond; deps; rev_deps } (target : Variable.t) =
  let eq =
    {
      eq_act = cond;
      eq_expr = (match form with Direct e -> e | Merge vars -> EMerge vars);
    }
  in
  if Condition.is_never cond then (acc, eq)
  else
    ...
    let acc =
      add_deps acc [ target ] cond_evts (Variable.Set.elements rev_deps)
    in
    (acc, eq)

let rec add_to_groups (groups : ('a list * Condition.t) list) (obj : 'a)
    (cond : Condition.t) =
  if Condition.is_never cond then groups
  else
    match groups with
    | [] -> [ ([ obj ], cond) ]
    | (gobjs, gcond) :: groups ->
        ...
        if Condition.is_never com then
          ...
          let groups =
            if Condition.is_never ex_gcond then groups
            else (gobjs, ex_gcond) :: groups
          in
          (obj :: gobjs, com) :: groups

let aggregate_exprs acc
    (exprs : (expr * Condition.t * Variable.Set.t * Variable.Set.t) list) =
  let ex_groups, target_cond =
    List.fold_left
      (fun (groups, glob_cond) (e, cond, deps, rdeps) ->
    ...
    List.map
      (fun (gexprs, gcond) ->
        let gexprs, depsl, rdepsl =
          List.fold_left
            (fun (es, ds, rs) (e, d, r) -> (e :: es, d :: ds, r :: rs))
            ([], [], []) gexprs
        in
        let gdeps =
          List.fold_left
            (fun s vs -> Variable.Set.union vs s)
  ...
  | [ (es, deps, rev_deps, cond) ] ->
      { acc; cond; deps; rev_deps; form = Direct (generate_addition es) }
  | _ ->
      let acc, fresh_vars =
        List.fold_left
          (fun (acc, fvars) (gexprs, gdeps, grdeps, gcond) ->
            ...
            let acc, fv =
              ...
              let acc =
                add_deps acc (Variable.Set.elements gdeps) evts [ fv ]
              in
              let acc =
                add_deps acc [ fv ] evts (Variable.Set.elements grdeps)
              in
              (acc, fv)
            in
...
let rec propagate_expr acc (dest : Variable.t) (act : Condition.t) (expr : expr)
  ...
  let sub_expr_1 e constr =
    ...
    in
    { acc; deps; rev_deps; form = Direct (constr e); cond = act }
  in
  ...
  | EAdd (e1, e2) ->
      ...
      in
      aggregate_exprs acc
        [ (e1, act1, deps1, rev_deps1); (e2, act2, deps2, rev_deps2) ]
...
and dispatch_eqs acc (dest : Variable.t) (aggr : aggregation) =
  ...
  | One eq ->
      ...
      if Condition.is_never eq.eq_act then (acc, no_existence)
      else (add_eq acc dest eq, eq)
  | More vars ->
      let acc, vars =
        List.fold_left_map
          (fun acc (v, cond) ->
...
and compute_one acc (v : Variable.t) =
  ...
  | None -> (
      ...
      | Some eq_expr -> affect_eq acc v { eq_expr; eq_act = Condition.always }
      | None -> (
          match Variable.Map.find_opt v acc.aggr with
          | None ->
              let eq_act =
                if VarInfo.is_input (find_vinfo acc v) then Condition.of_input v
...
let compute (pinfos : ProgramInfo.t) (ag_eqs : aggregate_eqs)
  ...
  let acc =
    Variable.Map.fold
      (fun dest _eqs acc -> fst @@ compute_one acc dest)
      ag_eqs acc
  in
  let acc =
    Variable.Map.fold
  ...
  let act_eqs, val_eqs =
    Variable.Map.partition (fun v _ -> is_event v) acc.memo
  in
  { infos = acc.pinfos; act_eqs; val_eqs; val_order; act_order }
