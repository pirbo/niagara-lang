...
module Acc : sig
  ...
  val register_input :
    t -> string -> ValueType.t -> input_kind -> t * Variable.t

  val register_pool : t -> string -> computed:bool -> t * Variable.t

  val register_value :
    t -> string -> obs:bool -> linear:bool -> typ:ValueType.t -> t * Variable.t

  val register_actor : t -> string -> t

  val register_actor_label :
    way:VarInfo.partner_role -> t -> string -> string -> t * Variable.t
  ...
  val project_var :
    t -> Variable.t -> Context.Group.t -> contextualized_variable
  ...
  type context_constraint = {
    ...
    projections : Context.Group.Set.t; (* User projection constraints *)
  }
  ...
  let register_input t (name : string) (typ : ValueType.t) (kind : input_kind) =
    match kind with
    | Attributable ->
        if typ <> ValueType.TMoney then
          Report.raise_internal_error "Wrong type for pool";
        let info =
          VarInfo.
            { origin = Named name; typ; kind = PoolInput { shadow = false } }
        in
  ...
  let register_actor t (name : string) =
    ...
    let info =
      VarInfo.{ origin = Named name; typ = TMoney; kind = Partner Receiver }
    in
    let t = bind_vinfo pv info t in
    let t = bind_linearity pv true t in
    bind_var name (RefActor (BaseActor pv)) t |> bind_compound pv pv

  let register_event t (name : string) =
    let v = Variable.create () in
    let info =
      VarInfo.{ origin = Named name; typ = ValueType.TEvent; kind = Event }
    in
    let t = bind_vinfo v info t in
  ...
  let register_const t (name : string) (typ : ValueType.t) (value : Literal.t) =
    let v = Variable.create () in
    let info = VarInfo.{ origin = Named name; typ; kind = Constant } in
    let t = bind_vinfo v info t in
    let t = bind_linearity v false t in
    bind_var name (RefConst v) t |> bind_const v value

  let project_var t (v : Variable.t) (proj : Context.Group.t) =
    if is_partner v t || is_constant v t then
      (v, Context.any_projection t.contexts)
    else (v, proj)
  ...
  let register_context_domain t (domain_name : string)
      (cases_names : string list) =
    { t with contexts = Context.add_domain t.contexts domain_name cases_names }

  let register_actor_label ~(way : VarInfo.partner_role) t (name : string)
      (label : string) =
    let base_actor =
      match StrMap.find_opt name t.var_table with
      ...
      | Some _ ->
          Report.raise_internal_error
            "'%s' should have been recognized as actor" name
    in
    ...
    | Some _ ->
        Report.raise_internal_error
          "'%s[%s]' should have been recognized as a labeled actor" name label

  let add_var_constraint t (v : Variable.t) (from_var : Variable.t)
      (proj : Context.Group.t) =
    if is_partner v t || is_constant v t then t
    else
      {
        t with
        constraints =
            ...
            (function
              | None ->
                  Some
                    {
                      from_vars =
                        Variable.Map.singleton from_var
                  ...
                  Some
                    {
                      constrs with
                      from_vars =
                        Variable.Map.update from_var
                          (function
                            | None -> Some (Context.Group.Set.singleton proj)
                            | Some projs ->
                                Some (Context.Group.Set.add proj projs))
                          constrs.from_vars;
                    })
            t.constraints;
      }
                  ...
                  Some
                    {
                      constrs with
                      projections =
                        Context.Group.Set.add proj constrs.projections;
                    })
            t.constraints;
      }
    ...
    let rec resolve_var v shapes =
      ...
      | None -> (
          ...
          | None ->
              ...
              (Variable.Map.add v everything shapes, everything)
          | Some { from_vars; projections } ->
             ...
             poured in some cases. *)
              ...
              in
              let filtered_shape =
                Context.shape_cut_out shape_from_vars forbidden
              in
             ...
             ouside *)
              let shape_with_projs =
                Context.Group.Set.fold
                  (fun p s -> Context.shape_imprint_projection s p)
                  projections filtered_shape
              in
              (Variable.Map.add v shape_with_projs shapes, shape_with_projs))
    in
    Variable.Map.fold
      (fun v _ shapes -> fst @@ resolve_var v shapes)
  ...
  let add_deps_from t (src : Variable.t) (dests : Variable.t list) =
    if VarInfo.is_partner (Variable.Map.find src t.var_info) then
      ...
      in
      if Variable.Set.mem src tdeps then Report.raise_error "Cyclic repartition";
      let deps =
    ...
    List.fold_left
      (fun proj ctx ->
        ...
        | Forall dom ->
            ...
            Context.DomainMap.update dom
              (function
                | Some _ (* TODO warning *) | None -> Some Context.CaseSet.empty)
              proj
        | Cases (dom, cases) ->
            ...
            let cases =
              List.fold_left
                (fun cases c ->
                  ...
                  if Context.case_is_in_domain contexts case domain then
                    Context.CaseSet.add case cases
                  else
                    Report.raise_error "Case %s do not belong in domain %s" c
                      dom)
                Context.CaseSet.empty cases
            in
            Context.DomainMap.update domain
              (function
                | Some excases ->
                    (* TODO warning *)
...
let find_actor ~(way : VarInfo.partner_role) acc (a : actor) =
  ...
  | LabeledActor (name, label) -> Acc.register_actor_label ~way acc name label
...
let find_holder0 ~(way : VarInfo.partner_role) acc (h : holder) =
  ...
  | Pool (name, ctx) ->
      ...
      | Some v -> (Acc.add_proj_constraint acc v proj, (v, proj))
      | None ->
...
let named acc (named : named) ~(on_proj : Context.Group.t) =
  ...
  in
  let proj =
    if Context.is_any_projection (Acc.contexts acc) proj then on_proj else proj
  in
...
let binop_typ (op : Surface.Ast.binop) (t1 : ValueType.t) (t2 : ValueType.t) =
  ...
  | Div, TMoney, TInteger
  | Div, TMoney, TRational ->
      ValueType.TMoney
  | Add, TDate, TDuration | Add, TDuration, TDate | Sub, TDate, TDuration ->
      ValueType.TDate
...
let rec formula acc ~(for_ : Variable.t option) (f : source formula)
  ...
  | Binop (op, f1, f2) ->
      ...
      let typ = binop_typ op t1 t2 in
      {
        ...
        is_input_linear =
          (match op with Add | Sub -> l1 && l2 | Mult | Div -> l1 || l2);
      }
  | Total f ->
...
and opposable acc ~(on_proj : Context.Group.t)
  ...
  let opp_towards =
    ...
    | LabeledActor _ ->
        Report.raise_error "Opposition target must be a partner without label"
...
let redistribution acc ~(for_ : Variable.t option)
    (redist : source redistribution) ~(on_proj : Context.Group.t)
    ~(when_guarded : bool) =
  ...
  | Retrocession (f, p) ->
      ...
      let right_operand =
        Surface.Ast.formula (Named (Surface.Ast.named (Holder p)))
      in
...
let redist_with_dest ~(src : Variable.t) ~(default_dest : Variable.t option) acc
    (WithHolder (redist, dest) : source redistrib_with_dest)
  ...
  in
  let for_ =
    let o = Option.fold ~none:default_dest ~some:(fun (d, _) -> Some d) dest in
...
let rec guarded_obj : type src_obj ctx_obj.
    Acc.t ->
    (Acc.t ->
    src_obj ->
    ...
    when_guarded:bool ->
    Acc.t * ctx_obj) ->
    (source, src_obj) guarded_redistrib ->
    on_proj:Context.Group.t ->
    when_guarded:bool ->
      ...
      let acc, befores =
        List.fold_left_map
          (fun acc (c, r) ->
            ...
            let acc, g_redist =
              guarded_obj acc obj_process r ~on_proj ~when_guarded
            in
            (acc, (cond, g_redist)))
          acc befores
      in
...
let operation acc (op : operation_decl) =
  ...
  let on_proj = projection_of_context_selector acc op.op_context in
  let acc, default_dest = destination_opt acc op.op_default_dest in
  let redist_process acc rs ~on_proj ~when_guarded =
    List.fold_left_map
      (fun acc r ->
        let acc, red_wd =
          redist_with_dest ~src
            ~default_dest:(Option.map fst default_dest)
  ...
  let acc, g_redist =
    guarded_obj acc redist_process op.op_guarded_redistrib ~on_proj
      ~when_guarded:false
  in
  let acc =
...
let event_decl acc (e : event_decl) =
  ...
  (acc, { ctx_event_var = v; ctx_event_expr = expr })
...
let comp_pool acc (p : comp_pool_decl) =
  ...
  in
  let on_proj = projection_of_context_selector acc p.comp_pool_context in
  let acc = Acc.add_proj_constraint acc pool on_proj in
  let acc, g_redist =
    guarded_obj acc
      (fun acc f ~on_proj ~when_guarded ->
        let { acc; formula; is_input_linear; typ } =
...
let declaration acc (decl : source declaration) =
  ...
  | DHolderDefault d ->
      ...
      in
      let acc, ctx_default_dest = destination acc d.default_dest in
      let acc =
        Acc.add_var_constraint acc src (fst ctx_default_dest) src_proj
      in
      let acc =
        Acc.add_deps_from acc (fst ctx_default_source) [ fst ctx_default_dest ]
      in
      Acc.add_program_decl acc
        (DVarDefault { ctx_default_source; ctx_default_dest })
  | DHolderDeficit d ->
      let acc, ctx_deficit_pool = find_holder acc d.deficit_pool in
      let acc, ctx_deficit_provider =
        find_holder_as_source acc d.deficit_provider
      in
      let acc =
        Acc.add_deps_from acc (fst ctx_deficit_provider)
          [ fst ctx_deficit_pool ]
      in
      Acc.add_program_decl acc
        (DVarDeficit { ctx_deficit_pool; ctx_deficit_provider })
