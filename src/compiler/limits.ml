...
type val_exprs = Zero | Exprs of guarded_eq list
...
let test_cartesian_limit vs1 vs2 =
  ...
  let c =
    ...
    | _ -> 0
  in
  if c > limit then raise (CombinatorialLimit c) else ()
...
let print_val_exprs fmt vl =
  ...
  | Exprs es ->
      fprintf fmt "@[<v 2>exprs:@ %a@])"
        (pp_print_list (fun fmt ge ->
             fprintf fmt "{%a} %a" Condition.print ge.eq_act
...
let add_limits acc (v : Variable.t) (l : threshold) =
  let cond_match c b = not @@ Condition.(is_never (conj c (of_event v b))) in
  let fl =
    match l with
    | Dynamic -> l
    | Static tl ->
        Static
          (List.filter
             ...
             tl)
  in
  { acc with lim_memo = Variable.Map.add v fl acc.lim_memo }

let add_val acc (v : Variable.t) (va : val_exprs) =
  { acc with val_memo = Variable.Map.add v va acc.val_memo }
      ...
      let exprs =
        List.filter_map
          (fun ge ->
            ...
            if Condition.is_never eq_act then None else Some { ge with eq_act })
          es
      in
...
let map_val f (v : val_exprs) =
  ...
  | Zero -> Zero
  | Exprs es ->
      Exprs (List.map (fun ge -> { ge with eq_expr = f ge.eq_expr }) es)
                   ...
                   (fun ge1 ->
                     List.filter_map
                       (fun ge2 ->
                         ...
                         if Condition.is_never eq_act then None
                         else
                           Some
                             {
        ...
        List.fold_left
          (fun (acc, vl) v ->
            ...
            let vals =
              match (vl, vals) with
              | Zero, vl | vl, Zero -> vl
              | Exprs ge1, Exprs ge2 -> Exprs (ge1 @ ge2)
            in
            (acc, vals))
          (var_val acc v) vars
      in
...
and var_val acc (v : Variable.t) =
  ...
  | None -> (
      if is_input acc v then
        ...
        | None ->
            Report.raise_error
              "Unable to compute value for variable %d, equation not found"
              (Variable.uid v))

type linear = { factor : (Variable.t * expr) option; const : expr option }

let rec linear_of_expr (e : expr) =
  match e with
  | EVar v -> { factor = Some (v, EConst (LRational R.one)); const = None }
  | EPre _ | EConst _ -> { factor = None; const = Some e }
  | ENeg e ->
...
let threshold_of_linears (cond : Condition.t) (l1 : linear) (l2 : linear) =
  ...
  let with_const var f edge c =
    Option.map
      (fun c ->
        { var; edge; value = { eq_act = cond; eq_expr = EMult (c, EInv f) } })
      c
  in
  match (l1.factor, l2.factor) with
  | None, None -> None
  | Some (v1, e1), Some (v2, e2) ->
      Format.eprintf "{v%d}%a = {v%d}%a@." (Variable.uid v1)
        FormatEqu.print_expr e1 (Variable.uid v2) FormatEqu.print_expr e2;
      Report.raise_error
        "Unable to properly solve limits with factors on both sides"
           ...
           (fun v1 ->
             List.filter_map
               (fun v2 ->
                 ...
                 if Condition.is_never cond then None
                 else
...
let rec expr_limits acc (evt : Variable.t) (e : guarded_eq) =
  ...
  | EGe (e1, e2) ->
      let acc, thresholds =
        ...
        with CombinatorialLimit _ -> (acc, Dynamic)
      in
...
let compute (p : program) =
  ...
  in
  let acc =
    Variable.Map.fold (fun evt eq acc -> expr_limits acc evt eq) p.act_eqs acc
  in
  acc.lim_memo
