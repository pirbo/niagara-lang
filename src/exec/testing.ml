...
let find_partner (infos : VarInfo.collection) (name : string) =
  let filter_info info =
    ...
    | Partner _ -> (
        match info.origin with Named n -> String.equal name n | _ -> false)
    | _ -> false
  in
...
let test_stdin (p : Equ.program) (l : Equ.limits) (for_partner : string option)
  ...
  let lines =
    IntMap.fold
      (fun i _ is -> IntMap.add i Results.AllSteps is)
      inputs IntMap.empty
  in
  let norm_mode =
    if for_all then Results.SquashAllButPartners
    else
      ...
      | Some s -> (
          ...
          | Some for_partner ->
              let relevancy_check =
                let rs =
                  match
                    Variable.Map.find_opt for_partner p.infos.relevance_sets
                  with
                  | None ->
                      Report.raise_error "No relevant set for partner %s" s
                  | Some rs -> rs
                in
  ...
  Interpreter.Printer.print_intepreter_outputs p
