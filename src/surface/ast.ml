...
and context_refinement_item_desc = CCase of string | CFullDomain of string

type context_refinement = context_refinement_item list

type actor = { actor_loc : Pos.t; actor_desc : actor_desc }
and actor_desc = PlainActor of string | LabeledActor of string * string

type holder = { holder_loc : Pos.t; holder_desc : holder_desc }
and holder_desc = Pool of string * context_refinement | Actor of actor

type named = { named_loc : Pos.t; named_desc : named_desc }
and named_desc = Name of string * context_refinement | Holder of holder

type contextualized_variable = Variable.t * Context.Group.t

type 'a formula = { formula_loc : Pos.t; formula_desc : 'a formula_desc }
...
type _ redistrib_with_dest =
  | WithHolder :
      source redistribution * holder option
      -> source redistrib_with_dest
  | WithVar :
      contextualized redistribution * contextualized_variable option
...
type context = Forall of string | Cases of string * string list
...
type const_decl = { const_name : string; const_value : Literal.t }
...
type actor_decl = { actor_decl_loc : Pos.t; actor_decl_desc : string }
type default_decl = { default_source : holder; default_dest : holder }
...
type deficit_decl = { deficit_pool : holder; deficit_provider : holder }
...
type ctx_comp_pool_decl = {
  ...
  ctx_comp_pool_guarded_value :
    (contextualized, contextualized formula) guarded_redistrib;
}
...
type _ program =
  ...
  | Contextualized :
      ProgramInfo.t * contextualized declaration list
      -> contextualized program

(* constructors with locations *)

let actor ?(loc = Pos.dummy) desc = { actor_loc = loc; actor_desc = desc }
let holder ?(loc = Pos.dummy) desc = { holder_loc = loc; holder_desc = desc }
let named ?(loc = Pos.dummy) desc = { named_loc = loc; named_desc = desc }
let formula ?(loc = Pos.dummy) desc = { formula_loc = loc; formula_desc = desc }

let redistribution ?(loc = Pos.dummy) desc =
  { redistribution_loc = loc; redistribution_desc = desc }

let event_expr ?(loc = Pos.dummy) desc =
  { event_expr_loc = loc; event_expr_desc = desc }

let actor_decl ?(loc = Pos.dummy) desc =
  { actor_decl_loc = loc; actor_decl_desc = desc }
...
let event_decl ?(loc = Pos.dummy) name expr =
  { event_loc = loc; event_name = name; event_expr = expr }

let context_refinement_item ?(loc = Pos.dummy) desc =
  { cri_loc = loc; cri_desc = desc }

let operation_decl ?(loc = Pos.dummy) ?default_dest ?(context = []) ~source
    ~guarded_redistrib label =
  {
    op_loc = loc;
    op_label = label;
