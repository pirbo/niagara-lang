...
let find_var (infos : VarInfo.collection) info_filter =
  let vars = Variable.Map.filter (fun _ info -> info_filter info) infos in
  match Variable.Map.cardinal vars with
  | 0 -> None
  | 1 -> Some (fst (Variable.Map.choose vars))
  | _ -> None

let find_input (infos : VarInfo.collection) (name : string)
    (ctx : Context.Group.t) =
  let filter_info info =
    if not (VarInfo.is_input info) then false
    else
...
let to_interpreter_input (pinfos : ProgramInfo.t) (i : raw) : input_line =
  let ctx =
    List.map
      (fun c -> Surface.Ast.{ cri_desc = CCase c; cri_loc = Pos.dummy })
      i.context
  in
  let var =
    match
      find_input pinfos.var_info i.name
        (Compiler.Contextualize.projection_of_context_refinement pinfos.contexts
           ctx)
    with
    | None ->
        Report.raise_error
          "Unable to find variable for input %d. Make sure you state the right \
           context"
          i.id
    | Some var -> var
  in
  let amount =
    ...
    | None -> (
        try Literal.LInteger (Z.of_string i.value)
        with Invalid_argument _ ->
          Report.raise_error "%s is not a valid amount" i.value)
  in
  { input_variable = var; input_value = amount }

let to_interpreter_inputs (pinfos : ProgramInfo.t) (is : raw list) :
    computation_inputs =
  List.fold_left
    (fun ci i -> InputLineMap.add i.id (to_interpreter_input pinfos i) ci)
    Execution.InputLineMap.empty is

(** Computes the largest convex domain from a list of domains. *)
let largest_domain (world : Context.world)
...
let of_interpreter_input (pinfos : ProgramInfo.t) (id : int) (i : input_line) :
    raw =
  let name =
...
let of_interpreter_inputs (pinfos : ProgramInfo.t) (ci : computation_inputs) :
    raw list =
  InputLineMap.fold
