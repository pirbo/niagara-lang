...
(* The interpreter uses a stack of action to make during a stage. This is to
   ...
   futur extensions. *)
type action = PoolRep of Variable.t * Value.t
...
let update_value (s : state) (var : Variable.t) (value : value_presence) =
  { s with valuations = Variable.Map.add var value s.valuations }

let update_event (s : state) (evt : Variable.t) (b : bool) =
  { s with events = Variable.Map.add evt b s.events }
...
let push_rep_action (p : program) (s : state) (var : Variable.t)
  ...
  | Pool -> { s with queue = PoolRep (var, value) :: s.queue }

let value_inputs (p : program) (s : state) =
  let valuations =
    Variable.Map.fold
      (fun v infos vals ->
        if VarInfo.is_input infos then Variable.Map.add v Absent vals else vals)
...
let rec valuate (s : state) (e : expr) =
  match e with
  | EVar v | EPre v ->
      (* assumes proper ordering/duplication *)
      find_valuation s v
...
let compute_value (s : state) (cond_state : cond_state)
  ...
  | MaySat ->
      Report.raise_internal_error
        "Unable to check activation, missing var value"
  | Unsat -> Absent
...
let rec expr_threshold_linear (p : program) (cond_state : cond_state)
    (expr : expr) =
  match expr with
      ...
      let factor =
        ...
        | Some (v1, f1), Some (v2, f2) ->
            if not (Variable.equal v1 v2) then (
              Format.eprintf "two var factors for %a@." FormatEqu.print_expr
                expr;
              assert false)
...
let rec dyn_event_thresholds (p : program) (cond_state : cond_state)
    (evt_expr : expr) =
  match evt_expr with
...
let compute_dyn_threshold (p : program) (cond_state : cond_state)
    (evt : Variable.t) =
...
let find_event_threshold (p : program) (l : limits) (s : state)
    (cond_state : cond_state) =
  Variable.Map.fold
    (fun evt e_thres min_threshold ->
      let thres_l =
        match e_thres with
      ...
      let s, value =
        ...
        | Some threshold ->
            if Value.lt threshold value then
              (push_rep_action p s var (Value.sub value threshold), threshold)
            else (s, value)
      in
...
let init_state (p : program) =
  ...
  let const_init vals =
    Variable.Map.fold
      (fun c l vals -> Variable.Map.add c (Present (literal_value l)) vals)
  ...
  in
  {
    ...
    valuations = init_map p.val_eqs (Present Value.zero) |> const_init;
    queue = [];
  }
