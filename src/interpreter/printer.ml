...
let print_event_switch fmt (changes : (string * bool) Variable.Map.t) =
  let open Format in
  if Variable.Map.is_empty changes then fprintf fmt "no events"
  else
    fprintf fmt "@[<hov>%a@]"
      (pp_print_list (fun fmt (_, (n, b)) ->
           fprintf fmt "%s event %s " (if b then "after" else "before") n))
...
let print_repartition ?(default = false) fmt (dest, value) =
  ...
  Format.fprintf fmt "%a -> %s" Value.human_print value dest

let print_default fmt rep = print_repartition ~default:true fmt rep

let rec print_item ?(close = true) infos fmt layout step
    (item : Results.top_item) =
  let open Format in
  let map_reps reps =
    let vals =
      Variable.Map.fold
        (fun dest values vals ->
          Variable.Map.fold
            (fun _stage_value rep_value vals ->
          ...
          | None -> (
              ...
              | None ->
                  Report.raise_internal_error
                    "Unable to find suitable name for repartition destination \
                     variable")
        in
        (name, v) :: replist)
      vals []
  ...
  | Top item | Detail item -> (
      ...
      | Absent -> false
      | Present at_step ->
          fprintf fmt "@[<v 2>- %s { %a, %a }:" item.display_name
          ...
          if List.length reps > 0 then
            fprintf fmt "@,%a" (pp_print_list print_repartition) reps;
          let defaults = map_reps item.defaults in
          if List.length defaults > 0 then
            fprintf fmt "@,%a" (pp_print_list print_default) defaults;
  ...
  | Flat item -> (
      ...
      | Absent -> false
      | Present value ->
          fprintf fmt "@[<v 2>- %s : %a -> %a" item.flat_name Value.human_print
            value
            (fun fmt target ->
          ...
          true)
  | Super item ->
      if print_item ~close:false infos fmt layout step (Top item.super_item)
      then begin
...
let print_step infos layout ~iter_vars fmt (step : output_step) =
  iter_vars (fun (item : Results.top_item) ->
      if print_item infos fmt layout step item then Format.pp_print_cut fmt ())

let print_event_line infos layout ~iter_vars ~changes fmt step =
  Format.fprintf fmt "@[<v 2>++ %a:@ %a@]@," print_event_switch changes
    (print_step infos layout ~iter_vars)
    step
...
let print_intepreter_outputs (p : Dataflow.Equ.program) fmt
    (norm_mode : Results.norm_mode) (lines : computation_outputs) =
  ...
  let _ =
    IntMap.fold
      (fun i line past_events ->
        fprintf fmt "%d: @[<v>" i;
        let changes =
          List.fold_left
            (fun past_events step ->
              let changes =
                event_flips var_infos past_events step.step_events
              in
              print_event_line p.infos.var_info layout ~iter_vars ~changes fmt
                step;
              step.step_events)
            past_events line
        in
