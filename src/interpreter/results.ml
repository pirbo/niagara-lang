...
let dummy_trigger v tr ta =
  { flat_name = "%no_name%"; value = v; trigger = tr; target = ta }

let update_flat_detail (v : Variable.t) (trigger : Variable.t option)
    (target : Variable.t) (f : flat_item -> flat_item) (layout : results_layout)
    =
...
let update_detail_of ?(is_detail = false) (v : Variable.t)
    (f : item_result_layout -> item_result_layout) (layout : results_layout) =
  Variable.Map.update v
    (function
      | None ->
...
let variant_copy (pinfos : ProgramInfo.t) layout
  ...
  Variable.Map.fold
    (fun target variants (layout : results_layout) ->
      ...
      let reps_update reps base =
        Variable.Map.fold
          (fun dest values reps ->
            let dest = variant_if_exists dest in
            Variable.Map.fold
              (fun value staged_v reps ->
                ...
                | Some value ->
                    Variable.Map.update dest
                      (function
                        ...
                        | Some values ->
                            Some (Variable.Map.add value staged_v values))
                      reps)
              values reps)
          reps base
      in
      ...
      let layout, variant_detail_items =
        Variable.Map.fold
          (fun org item (layout, details) ->
            match item with
            | Top item ->
                let canon_item = update_canon_item item in
                let layout = Variable.Map.add org (Top canon_item) layout in
                (layout, details)
            | Detail item ->
                let canon_item = update_canon_item item in
                let layout = Variable.Map.add org (Detail canon_item) layout in
                (layout, details)
            | Flat item ->
                let layout = Variable.Map.add org (Flat item) layout in
                (layout, details)
            | Super { super_item; super_detail_items } -> (
                let canon_super_item = update_canon_item super_item in
                let layout =
                  Variable.Map.add org
                    (Super { super_item = canon_super_item; super_detail_items })
                    layout
                in
                ...
                | Some variant ->
                    ...
                    in
                    ( Variable.Map.add variant
                        (Super { super_item; super_detail_items })
                        layout,
                      Variable.Set.union details super_detail_items )))
          layout
          (layout, Variable.Set.empty)
      in
      Variable.Map.fold
        (fun org item layout ->
          match Variable.Map.find_opt org variants with
          ...
          | Some variant -> (
              ...
              | Flat item ->
                  Variable.Map.add variant
                    (Flat (copy_flat_item item variant))
                    layout
              | Detail item ->
                  let item =
                    if Variable.Set.mem variant variant_detail_items then
                      Detail (copy_item item variant)
    ...
    Variable.Map.fold
      (fun v infos (layout, variants) ->
        ...
        | Named name -> (
            ...
            | Event | Constant | Value { observable = false; _ } ->
                (layout, variants)
            | Value { cumulative; _ } ->
                ( update (fun l ->
                      {
        ...
        | LabelOfPartner { partner; label } ->
            ( super_update partner (fun l ->
              ...
              variants )
        | Cumulative step ->
            ( update_detail_of step
                (fun l -> { l with cumulated = Some v })
                layout,
              variants )
        ...
        | OperationDetail { label; op_kind; source; target; condition = _ } ->
            ( (match op_kind with
              ...
              | Bonus _ ->
                  let layout =
                    update_flat_detail v None target
                      (fun l ->
                        {
                          l with
                          flat_name = Option.value label ~default:"flat bonus";
                        })
                      layout
                  in
                  update_detail_of source
              ...
              variants )
        | TriggerOperation
            { label; source = _; target; trigger; trigger_vars = _ } ->
            let layout =
              update_detail_of target
                (fun l -> { l with computed = Variable.Set.add v l.computed })
                layout
            in
            ( update_flat_detail v (Some trigger) target
                (fun l ->
                  {
                    l with
                    flat_name = Option.value label ~default:"triggered bonus";
                  })
                layout,
              variants )
            ...
            ( update (fun l ->
                  {
                    l with
                    display_name =
                      VarInfo.get_any_name pinfos.var_info target ^ " delta";
                  }),
              variants )
...
let sort_layout ~(graph : Variable.Graph.t) (layout : results_layout) =
  ...
  List.filter_map
    (fun v ->
      ...
      Option.bind item (function Detail _ -> None | i -> Some i))
    order
...
type line_squashing = MeldInNext | SquashSteps | AllSteps
...
let filter_of_norm_mode (info : ProgramInfo.t) (mode : norm_mode) =
        ...
        (fun v vinfos partners ->
          ...
          | OpposingVariant { origin; target; _ }, _ ->
              ...
              let rec own_partner org vinfo =
                ...
                | Cumulative s ->
                    own_partner s (Variable.Map.find s info.var_info)
  ...
  | Explain
      ...
      let line_filter i =
        match IntMap.find_opt i lines with Some s -> s | None -> MeldInNext
      in
      let rec org_check v =
        let vinfo = Variable.Map.find v info.var_info in
        let rec variant_check variant =
          ...
          | RepartitionSum _ | DeficitSum _ | OperationDetail _ | OperationSum _
          | TriggerOperation _ ->
      ...
      | _ -> (
          ...
          | Some p1, Some p2 ->
              let rec merge_on_org vorigin =
                match vorigin with
                | Cumulative _ -> p2
                | Named _ | LabelOfPartner _ | Peeking _ | ContextSpecialized _
                | OperationDetail _ | TriggerOperation _ | LocalValuation _
                | OperationSum _ | RepartitionSum _ | DeficitSum _
...
let normalize_valuations (info : ProgramInfo.t) (mode : norm_mode)
  ...
  let filter_step step =
    {
      step with
      step_events =
        Variable.Map.filter (fun v _ -> var_filter v) step.step_events;
    }
  in
  ...
  let push_line i pending line vals =
    ...
    if line = [] then vals else InputLineMap.add i line vals
  in
  ...
  let line_steps ~i ~squashing pending line =
    ...
    | AllSteps ->
        let rsteps =
          List.fold_left
            (fun rsteps step -> try_merge_rev_step step rsteps)
            [] line
        in
  ...
  let pending, vals =
    InputLineMap.fold
      (fun i line (pending, vals) ->
        let squashing = line_filter i in
        let line_steps, pending = line_steps ~i ~squashing pending line in
        if squashing = MeldInNext then (pending, vals)
        else (None, push_line i pending line_steps vals))
      valuations (None, InputLineMap.empty)
  in
  push_pending pending vals

let normalize_layout (info : ProgramInfo.t) (mode : norm_mode)
    (layout : results_layout) : results_layout =
    ...
    let filter_map map =
      Variable.Map.filter_map
        (fun _d vs ->
          ...
          if Variable.Map.is_empty vs then None else Some vs)
        map
    in
    ...
    Variable.Map.fold
      (fun v item (nlayout, details) ->
        ...
        let nlayout, details =
          ...
          | Flat item ->
              let nlayout =
                if show then Variable.Map.add v (Flat item) nlayout else nlayout
              in
              (nlayout, details)
        in
        if show then (nlayout, details)
        else (nlayout, Variable.Set.add v details))
      layout
      (Variable.Map.empty, Variable.Set.empty)
  in
  Variable.Map.fold
    (fun v item layout ->
      match item with
      ...
      | (Top i | Super { super_item = i; _ }) when Variable.Set.mem v as_details
        ->
