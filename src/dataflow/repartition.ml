...
let add_upart (uni : unified_parts) (cond : Condition.t) (part : R.t) =
  R.Map.update part
    (function
      | None -> Some cond | Some ucond -> Some (Condition.disj ucond cond))
    uni
  ...
  let rem_cond, rep =
    R.Map.fold
      (fun p pc (cond, rep) ->
        ...
        if Condition.is_never com then (cond, add_upart rep pc p)
        else
          let rep_ex = Condition.excluded pc com in
          let cond_ex = Condition.excluded cond com in
          let rep = add_upart rep com R.(p + part) in
          let rep =
            if Condition.is_never rep_ex then rep else add_upart rep rep_ex p
          in
          (cond_ex, rep))
      rep (cond, R.Map.empty)
  in
  if Condition.is_never rem_cond then rep else add_upart rep rem_cond part

let default_parts (rep : unified_parts) (def_cond : Condition.t) =
  let defaults, rem_cond, rep =
    R.Map.fold
      (fun p pc (defs, cond, rep) ->
        let com = Condition.conj cond pc in
        if Condition.is_never com then (defs, cond, add_upart rep pc p)
        else
          let cond_ex = Condition.excluded cond com in
          if R.(p >= one) then (defs, cond_ex, add_upart rep pc p)
          else
            let rep_ex = Condition.excluded pc com in
            let rep =
              if Condition.is_never rep_ex then rep else add_upart rep rep_ex p
      ...
      rep
      (R.Map.empty, def_cond, R.Map.empty)
  in
  if Condition.is_never rem_cond then (defaults, rep)
  else (add_upart defaults rem_cond R.one, add_upart rep rem_cond R.one)

let deficit_parts (rep : unified_parts) (def_cond : Condition.t) =
  R.Map.fold
    (fun p pc (defs, rep) ->
      let com = Condition.conj def_cond pc in
      if Condition.is_never com then (defs, add_upart rep pc p)
      else if R.(p <= one) then (defs, add_upart rep pc p)
      else
        let rep_ex = Condition.excluded pc com in
        let rep =
          if Condition.is_never rep_ex then rep else add_upart rep rep_ex p
            ...
            if Condition.is_always share.condition then
              ...
              match
                List.find_opt
                  (fun sh ->
                    not
                    @@ Condition.(is_never (conj share.condition sh.condition)))
                  defs.local_defaults
              with
              | Some _ ->
                  Report.raise_error
                    "Cannot have coexisiting local defaults for a pool"
              | None ->
  ...
  let parts_cond =
    R.Map.fold
      (fun p pc cond ->
        ...
        if R.(p < one) then raise (Stop (ImperfectSum p))
        else if R.(p > one) then raise (Stop (ImperfectSum p))
        else cond)
      rep Condition.never
  in
  let rem_cond = Condition.(excluded always parts_cond) in
  if not @@ Condition.is_never rem_cond then raise (Stop (ImperfectSum R.zero))
  else ()
...
let resolve_fullness_exn (rep : part_or_def t) =
  ...
  let parts, deficit =
    ...
    | Some share ->
        ...
        let ds =
          R.Map.fold
            (fun op cond ds -> add_part ds cond op)
  ...
  check_fullness parts;
  {
    parts =
      List.filter_map
        (fun { label; part; dest; condition; main_event } ->
          match part with
...
let resolve_fullness (rep : part_or_def t) =
  try Ok (resolve_fullness_exn rep) with Stop err -> Error err
