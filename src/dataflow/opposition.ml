  ...
  let one_copy acc var =
    let copies =
      Variable.Map.update var
        (function
          | None | Some None -> Some (Some (Variable.create ())) | c -> c)
        acc.copies
    in
    { acc with copies }
  in
...
let register_consequence acc env (var : Variable.t) (is_consequent : bool) =
  if is_consequent then add_copy acc env var else not_consequent acc env var
...
let rec graph_edge acc g (from : Variable.t) (to_ : Variable.t) =
  ...
  if from_present then g else graph_var acc g from

and graph_expr acc g (succ : Variable.t) (expr : expr) =
  match expr with
  | EConst _ -> g
  | EVar var | EPre var -> graph_edge acc g var succ
  | ENeg e | EInv e | ENot e -> graph_expr acc g succ e
  | EAdd (e1, e2) | EMult (e1, e2) | EAnd (e1, e2) | EGe (e1, e2) ->
...
and graph_eq acc g (succ : Variable.t) (eq : aggregation) =
  ...
  | More vars ->
      List.fold_left (fun g (var, _cond) -> graph_edge acc g var succ) g vars
...
let populate_copies acc env g =
  let oppositions_reach =
    Variable.Map.fold
      ...
      env.user_substs Variable.Set.empty
  in
  let acc =
    Variable.Graph.fold_vertex
      (fun v acc ->
        register_consequence acc env v (Variable.Set.mem v oppositions_reach))
      g acc
  in
  (acc, not @@ Variable.Set.is_empty oppositions_reach)
...
let condition_subst csq (cond : Condition.t) =
  ...
  let rec aux (tree : tree) =
    ...
    | Branch { var = Event ev; yes; no } ->
        ...
        disj (conj (of_event ev true) yes) (conj (of_event ev false) no)
  in
  aux (tree cond)
...
let origin_variant acc env (var : Variable.t) (vorigin : VarInfo.origin) =
  ...
  let convert_reps (src : Variable.t) (rep : Condition.t R.Map.t) =
    ...
    in
    let add_rep c r rep =
      if R.(r < zero || r > one) then rep
      else R.Map.add r (condition_subst acc c) rep
    in
    R.Map.fold
      (fun r c rep ->
        Variable.Map.fold
          (fun _ subst rep ->
            ...
            if Condition.is_never ccond then add_rep c r rep
            else
              let xcond = Condition.excluded c subst.condition in
              let cr = R.(r - delta) in
              let rep = add_rep ccond cr rep in
              if Condition.is_never xcond then rep else add_rep xcond r rep)
          relevant_substs rep)
      rep R.Map.empty
  in
  ...
  in
  match vorigin with
  | Named _ | AnonEvent | Peeking _ | RisingEvent _ | ContextSpecialized _
  | ConditionExistential | OppositionDelta _ ->
      vorigin
  ...
  Variable.Map.fold
    (fun org dup acc ->
      ...
      | Some dup ->
          ...
          let origin =
            VarInfo.OpposingVariant
              { target = env.target; origin = org; variant }
          in
          let acc =
            {
              acc with
          ...
          if is_event then duplicate_event acc env ~org ~dup
...
let add_delta acc env =
  ...
  | Some (Some opp) ->
      ...
      let cdvinfo =
        VarInfo.{ kind = Intermediary; typ = TMoney; origin = Cumulative dv }
      in
      ...
      let cdve = EAdd (EPre cdv, EVar dv) in
      {
        ...
        copies =
          ...
          Variable.Map.add dv None acc.copies |> Variable.Map.add cdv None;
        var_info =
          Variable.Map.add dv dvinfo acc.var_info
          |> Variable.Map.add cdv cdvinfo;
        value_eqs =
          Variable.Map.add dv
            (One { eq_act = Condition.always; eq_expr = dve })
            acc.value_eqs
          |> Variable.Map.add cdv
               (One { eq_act = Condition.always; eq_expr = cdve })
    ...
    ProgramInfo.
      {
        endpoint =
          ...
          | None | Some None ->
              if opposable then
                Report.raise_internal_error
                  "No opposing variant for opposability target"
              else target
          | Some (Some var) -> var);
        relevant_vars =
          Variable.Map.fold
            (fun org dup set ->
              let pv = match dup with None -> org | Some dup -> dup in
              Variable.Set.add pv set)
            acc.copies Variable.Set.empty
          (* ensure upstream labels are included *)
          |> Variable.Map.fold
               (fun v (info : VarInfo.t) pset ->
...
let resolve_one_target ~opposable pinfos acc ~(target : Variable.t)
    ~(provider : Variable.t) (user_substs : user_substitutions)
    (cumulatives : Variable.t Variable.Map.t) =
  ...
  if (not has_consequent) && opposable then
    Report.raise_useless_opposition_error
      { pinfos with var_info = acc.var_info }
      target;
...
let resolve (pinfo : ProgramInfo.t) (value_eqs : aggregate_eqs)
    (event_eqs : expr Variable.Map.t)
    (oppositions : user_substitutions Variable.Map.t)
    ~(cumulatives : Variable.t Variable.Map.t)
    ~(providers : Variable.t Variable.Map.t) =
  let acc =
    {
      var_info = pinfo.var_info;
      value_eqs;
      event_eqs;
      copies = Variable.Map.empty;
  ...
  let acc =
    Variable.Map.fold
      (fun target substs acc ->
        let provider =
          match Variable.Map.find_opt target providers with
          | None ->
              Report.raise_internal_error
                "no provider for opposition target '%s'"
                (VarInfo.get_any_name acc.var_info target)
          | Some provider -> provider
        in
        resolve_one_target ~opposable:true pinfo acc ~target ~provider substs
          cumulatives)
      oppositions acc
  in
  ...
  let acc =
    Variable.Map.fold
      (fun v t acc ->
        ...
        | None -> acc
        | Some _ ->
            if Variable.Map.mem v oppositions then acc
            else
              resolve_one_target ~opposable:false pinfo acc ~target:v
                ~provider:v (* should be useless *)
                Variable.Map.empty cumulatives)
      pinfo.var_info acc
  in
  {
