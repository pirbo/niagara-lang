...
let to_dec_repr (rat : Q.t) =
  ...
  let rec dec_list rest n =
    ...
    if r = zero || reach_loop then rest else dec_list rest (r * ~$10)
  in
  ...
  | (q, fr) :: rdec ->
      let fixdec, repeatend =
        List.fold_left
          (fun (fd, rp) (q, r) ->
            if fr = r then (to_string q, Some fd) else (to_string q ^ fd, rp))
          (to_string q, None)
          rdec
      in
...
let print_dec_approx fmt t =
  ...
  | [ _; sdec ] ->
      if String.length sdec > precision then
        Format.fprintf fmt "%a.%s..." Z.pp_print int
          (String.sub sdec 0 precision)
      else Format.fprintf fmt "%a.%s" Z.pp_print int sdec
