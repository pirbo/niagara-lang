...
module Memoizing = struct
  module Make (H : Hashtbl.HashedType) : sig
    ...
    let memo2 f =
      ...
      let rec ff k1 k2 =
        try Hash2.find ht (k1, k2)
        with Not_found ->
          let v = f ff k1 k2 in
          Hash2.add ht (k1, k2) v;
          v
      in
      ff
  end
...
type var = Input of Variable.t | Event of Variable.t
type comp = Eq | Lt | Gt | Contradicts of int

let compare2comp c = if c = 0 then Eq else if c < 0 then Lt else Gt

let var_comp v1 v2 =
  match (v1, v2) with
  | Input v1, Input v2 ->
      let c = Variable.compare v1 v2 in
      if c <> 0 then Contradicts c else compare2comp c
  | Event v2, Event v1 ->
      (* reversed for events *)
      let c = Variable.compare v1 v2 in
      compare2comp c
...
type bdd = T | F | Var of var * bdd hcd * bdd hcd
...
module HCBDD = Hashing.Make (struct
  ...
  let equal t1 t2 =
    ...
    | Var (Event v1, l1, r1), Var (Event v2, l2, r2) ->
        Variable.equal v1 v2 && l1 == l2 && r1 == r2
    | _ -> false
...
let var_weight var =
  match var with Input v -> Variable.uid v | Event v -> -Variable.uid v

let weight t =
  match get t with T | F -> max_int | Var (v, _, _) -> var_weight v

let node v l r =
  if var_weight v >= weight l || var_weight v >= weight r then assert false;
...
let neg =
  Memo.memo (fun neg t ->
      map
        (function
          | F -> always | T -> never | Var (v, l, r) -> node v (neg l) (neg r))
        t)

let of_bool b = if b then always else never

let no_more_inputs =
  Memo.memo (fun no_more_inputs ->
      map (function
        | T -> always
        | F -> never
        | Var (v, l, r) -> (
...
let comb op =
 fun comb l r ->
  ...
  | F, Var (v, l, r) | Var (v, l, r), F -> node v (comb l never) (comb r never)
  ...
  Memo.memo (fun past_of t ->
      ...
      | Var (v, l, r) -> (
          ...
          | _ -> (
              ...
              match get r_past with F -> never | _ -> node v l_past r_past)))
...
let print_var fmt v b =
  ...
  in
  if b then fprintf fmt "%c%d" v d else fprintf fmt "%a%c%d" print_neg () v d
...
let rec print fmt t =
  ...
  | Var (v, l, r) -> (
      ...
      | _ ->
          fprintf fmt "@[<hov 2>(%a@ %a %a@])" (print_with_var v true) l
            print_disj () (print_with_var v false) r)
...
and print_with_var v b fmt t =
  ...
  | (T | F), _ ->
      ...
      if (tv && b) || not (tv || b) then print_var fmt v true
  ...
  | _ ->
      fprintf fmt "@[<hov 2>(%a@ %a %a@])"
        (fun fmt -> print_var fmt v)
        b print_conj () print t
...
let time_ranking (ts : t list) : t list =
  let insert_rank ranks t =
    ...
    let rec aux ranks t =
      if is_never t then (never, ranks)
      else
        match ranks with
        | [] -> (never, [ t ])
        | r :: rs ->
            ...
            let r, t =
              if is_never bubble then
                if is_never (conj r t) then (never, after)
                else
...
type satisfaction = Sat | Unsat | MaySat
...
type tree = True | False | Branch of { var : var; yes : tree; no : tree }
...
let events_of (t : t) =
  let rec aux evts t =
    match get t with
    | T | F -> evts
    | Var (Event v, yes, no) -> aux (aux (Variable.Set.add v evts) yes) no
    | Var (Input _, yes, no) -> aux (aux evts yes) no
  in
