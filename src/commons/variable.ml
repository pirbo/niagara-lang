...
module Graph = struct
  ...
  let reachables (g : G.t) (start : G.V.t) : Set.t =
    let rec aux acc v =
      if Set.mem v acc || not (G.mem_vertex g v) then acc
      else List.fold_left aux (Set.add v acc) (G.succ g v)
    in
    aux Set.empty start
    ...
    let rev_topological_depth_ordering (g : G.t) : G.V.t list =
      let rec aux v ((mem, ord, hold) as acc) =
        if Set.mem v mem then acc
          ...
          let mem, ord, hold =
            List.fold_left (fun acc p -> aux p acc) acc (G.pred g v)
          in
          if G.in_degree g v = 0 then Set.fold aux hold (mem, ord, hold)
          else (mem, ord, hold)
        else (mem, ord, Set.add v hold)
      in
      let _, ord, _ =
        G.fold_vertex
          (fun v acc -> if G.out_degree g v = 0 then aux v acc else acc)
      ...
      List.fold_left
        (fun g v ->
          G.fold_pred_e
            (fun (p, e, _) g ->
              G.fold_pred_e
                (fun (pp, pe, _) g -> G.add_edge_e g (pp, Set.union e pe, v))
                g p g)
            g v g)
        g0 order
  end
