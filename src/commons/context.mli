...
type shape
(** A shape is a set of disjoint groups that represents the various aggregation
    of points in the context space. The space is not necessarily present in its
    entirety. *)
...
(** Representation of a group population. Each domain to a set of cases, the
    ...
    This representation might in some cases need to be fragmented, hence the
    list.

    An example:
    - Domain A: cases A1, A2 Domain B: case B1
    - Domain A: case A2 Domain B: case B2
    ...
    - Domain A: cases A1, A2 Domain B: cases B1, B2 represents the points (A1,
      B1), (A1, B2), (A2, B1) and (A2, B2), which is one too many.
...
(** [shape_filter_projection s g] returns a shape whose groups are the groups of
    [s] that are entirerly included in [g].
...
(** [shape_cut_out s g] returns a shape whose groups are the non-empty
    ...
    It is equivalent to
    [shape_filter_projection (shape_imprint_projection s g) g]. *)

val shape_of_groups : Group.t list -> shape
(** Creates a shape from groups. An error is thrown if the groups are not all
    disjoints. *)
...
(** Check whever the given group contains the entire space of the given world.
*)
