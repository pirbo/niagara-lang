...
let print_error (pinfo : ProgramInfo.t) fmt (err : error) =
  ...
  | Repartition { pool; rep } ->
      ...
      if R.(rep > one) then
        ...
        Format.fprintf fmt "Pool %S is too low (%g%%), needs a default"
          (VarInfo.get_any_name pinfo.var_info pool)
          R.(to_float (rep * ~$100))
  | MissingDestination -> Format.fprintf fmt "No destination for repartition"
  | UnknownIdentifier { loc; id; kind } ->
      Format.fprintf fmt "Error, %a:@\nUnknown %sidentifier '%s'" Pos.Text.pp
        loc
        (match kind with
        | Any -> ""
        | Event -> "event "
  ...
  | MultipleDefRep { pool } ->
      Format.fprintf fmt
        "Error: Cannot have several deficit/default for pool '%s'"
        (VarInfo.get_any_name pinfo.var_info pool)
  | Typing { loc } ->
      Format.fprintf fmt "Error, %a:@\nMismatching types" Pos.Text.pp loc
  | ForbiddenNonLinear { loc } ->
      Format.fprintf fmt
        "Error, %a:@\n\
         Non-linear expression forbidden, cannot ensure discretized continuity"
        Pos.Text.pp loc
  ...
  | MultipleOppositionProvider { locs; towards } ->
      Format.fprintf fmt "Error, %a:@\nSeveral opposition provider towards '%s'"
        ...
        locs
        (VarInfo.get_any_name pinfo.var_info towards)
  | User { locs = _; msg } -> Format.pp_print_string fmt msg

include Logs

type info = { pinfo : ProgramInfo.t; kind : error }
...
let raise_internal_error fmt =
  let k s = log_error "Internal error" ProgramInfo.dummy (Internal s) in
  Fmt.kstr k fmt
...
let raise_unknown_id_error ?(loc = Pos.dummy) id kind =
  log_error "Unknown identifier" ProgramInfo.dummy
    (UnknownIdentifier { loc; id; kind })
...
let raise_multiple_opp_provider_error ?(locs = []) pinfo towards =
  log_error "Useless opposition" pinfo
    (MultipleOppositionProvider { locs; towards })
