...
let lident =
  [%sedlex.regexp? lowercase, Star (digit | lowercase | uppercase | '_')]

let uident =
  [%sedlex.regexp? uppercase, Star (digit | lowercase | uppercase | '_')]
...
let parse_money_value s : Z.t option =
  try
    (* Catch conversion errors. Should not happen from the lexer itself. *)
    ...
    | [ intpart; decpart ] ->
        ...
        in
        if declen < 3 && declen >= 0 then Some Z.((intpart * ~$100) + ~$decpart)
        else None
    | _ -> None
  with _ -> None
...
let parse_money_amount s =
  ...
  match parse_money_amount_opt s with Some i -> i | None -> raise invalid_arg
...
let rec code ~is_in_text lexbuf =
  if not !reading_code then in_text lexbuf
  else
    ...
    | eof ->
        if is_in_text then
          Report.raise_parsing_error "Unclosed code section at end of file"
            ~loc:(Pos.Text.from_lexbuf lexbuf)
        else EOF
    | _ -> unknown_token_error lexbuf

and in_text lexbuf =
  if !reading_code then code ~is_in_text:true lexbuf
  else
    match%sedlex lexbuf with
    | "```niagara" ->
        reading_code := true;
        code ~is_in_text:true lexbuf
    | any -> in_text lexbuf
    | eof -> EOF
    | _ -> assert false
...
let text_and_code () =
  reading_code := false;
  code ~is_in_text:true

let program_file () =
  reading_code := true;
  code ~is_in_text:false
