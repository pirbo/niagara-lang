...
let fail checkpoint =
  ...
  | I.HandlingError env ->
      ...
      Report.raise_parsing_error
        ~loc:(Pos.Text.make ~start ~stop)
        (ParserErrors.message (I.current_state_number env))
  | _ -> assert false

let parse_lexbuf :
    entry:(Lexing.position -> 'a I.checkpoint) ->
    ?literate:bool ->
    Sedlexing.lexbuf ->
    ('a -> 'b) ->
    'b =
 fun ~entry ?(literate = false) lexbuf k ->
  let lexer =
    ...
    | false -> Lexer.program_file ()
  in
  let init_checkpoint = entry (fst @@ Sedlexing.lexing_positions lexbuf) in
  let program =
    I.loop_handle accept fail
      (Sedlexing.with_tokenizer lexer lexbuf)
      init_checkpoint
  in
  k program

let parse_program : string -> Ast.source Ast.program =
 fun filename ->
  ...
  try
    parse_lexbuf ~entry:Parser.Incremental.program ~literate lexbuf
      (fun program ->
        close_in file_chan;
        program)
  with exn ->
...
let parse_string :
    entry:(Lexing.position -> 'a I.checkpoint) -> ?literate:bool -> string -> 'a
    =
