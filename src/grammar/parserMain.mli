...
(** [parse_string ~entry ~literate str] returns the Niagara AST piece
    corresponding to the production of [str] through [entry]. [str] content is
    treated as Mardown if [literate] is [true] or as a Niagara source else.
...
val parse_gen :
  ...
  char Gen.t ->
  'a
(** [parse_gen ~entry ~literate gen] returns the Niagara AST piece read from the
